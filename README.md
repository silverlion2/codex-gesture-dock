...
- 摄像头工作区可切换姿态、QR/条码、文档快照、文件 OCR 与名片 OCR，并支持设备选择、镜像/原图和填满/完整预览
- 可独立选择并明确开启麦克风；输入电平只在本机内存中计算，不播放、不录制、不上传
- 文件 OCR 支持 PNG、JPEG、WebP 与最多 20 页 PDF；中英文 Tesseract 模型和 PDF worker 随应用离线打包
- 名片 OCR 会预填姓名、公司、职位、电话、邮箱、网站和地址；用户确认或修正后可导出标准 `.vcf` 联系人文件
- 扫码与 OCR 结果只留在内存；文档先预览，只有用户点击保存/导出才写入下载目录
