...
当前交付目标是“一键极简桌面控制”：Windows 手势模式优先提供六个固定动作，并通过动作级 callback 返回结果。六手势是切换下一个/上一个窗口、最小化当前窗口、最大化/还原当前窗口、任务视图和显示桌面；动作失败、急停、helper 不可用、UIPI 或前台限制都必须可见且失败关闭。资源管理器、左右贴靠和音量动作仍保留在固定白名单中。

当前不提供系统托盘、关闭到托盘、任意按键/文本、拖拽、窗口标题/句柄选择或通用多程序语义控制。`minimal` 是本应用的 78×78 非聚焦视图状态；系统切换使用固定 `SendInput` 组合键，当前前台窗口的最小化与最大化/还原则使用受限 Win32 调用并排除 Dock 进程。
...
2. **Windows Control Core**：一方面验证 `OpenAI.Codex` MSIX 身份、进程与前台窗口，只对验证通过的 Codex 发送固定语义动作；另一方面提供 12 个不针对特定程序的固定系统动作（窗口前后切换、最小化、最大化/还原、左右贴靠、显示桌面、任务视图、资源管理器和音量），以及用户明确选择后才启用的空中鼠标。三类动作共用持久化急停和限流。核心不接收任意按键、文本、脚本、窗口句柄或控件选择器；空中鼠标只接收当前显示器内的受限坐标、左键单击和固定步长滚动。
...
| Windows 系统动作 | 完成（v1） | 12 项固定 SendInput/Win32 动作；六手势映射；不接受调用方提供的按键序列或窗口标识 |
...
| Windows 系统动作 | 固定系统键/窗口 helper | 仅 12 项枚举动作；窗口动作只作用于当前前台窗口并排除 Dock PID |
...
- [已完成] 独立 Windows 手势模式暴露窗口前后切换、当前窗口最小化、最大化/还原、任务视图和显示桌面；资源管理器、左右贴靠及音量保留在固定白名单。主进程与 PowerShell helper 双重枚举校验，并通过动作 callback 回传结果。
