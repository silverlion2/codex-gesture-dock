...
## 0.6.2 发布候选 — 2026-09-19

- 授权：用户在 UI/UX 与默认仅手势改动验收后明确要求 publish；仅发布，本轮不另行安装到本机。
- 范围：六手势动作与摆法完整展示、Windows 优先布局、语音/电平说明分离、窄屏重叠修复；坐姿模型默认不加载，显式开启才校准。
- 版本：package/lockfile 同步 0.6.2；裸 semver tag，走既有未签名 all-releases 渠道，不降级签名正式发布门禁。
- 验证：重新执行 release 门禁；CI 从干净 main 构建，验证打包、安装/升级/卸载及七项资产后才能公开发布。记录最终工作流/资产证据于本地 `work/release-0.6.2-published.md`。
- 回滚：不覆盖 0.6.1 或 0.6.2 资产；故障时停止推广，手动恢复既有 0.6.1 安装包或发布更高补丁版本，不自动降级。
- 限制：无可信 Authenticode 签名；真人手势准确率、口音/噪声及权限边缘场景不由自动测试证明。`optimization_plan.md` 保留且不纳入提交。
