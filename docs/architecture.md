...
就绪链路分离摄像头/手势与坐姿校准；原生执行使用自有临时测试窗口验证真实输入，摄像头 smoke 只检查真实帧及模型加载且阻断桌面动作，六手势与误触/光照另由用户实机确认。

更新使用既有 electron-updater GitHub provider，不自建下载或执行协议：允许 prerelease，禁止 downgrade，保留内置校验和、签名验证及固定仓库。未来未签名渠道 tag 为裸 semver（如 `0.6.1`），签名正式渠道 tag 仍为 `v<version>`；两者资产都必须有一致 `latest.yml`、setup 和 blockmap。旧 `preview/v0.6.0` 既不满足 provider tag 解析也没有更新清单，保持历史不可变，不能靠客户端开关使它自动更新。

桌面控制采用：手势确认 → `runWindowsAction` IPC → 主进程枚举与限流 → 固定 helper → 动作结果 callback。callback 必须携带动作、成功/失败、失败原因和时间；UI 不得只依赖窗口监控事件推断动作是否成功。控制失败、helper 超时、权限不足或前台限制时进入可见失败态并保持急停边界。

六手势优先于扩展动作。`minimal` 改变本应用窗口与 React 布局，并把 Dock 设为不可聚焦，使系统级切换、最小化和最大化/还原作用于用户的前台窗口；没有托盘或任意窗口选择器。键盘组合使用固定参数的 `SendInput`，前台窗口最小化和最大化/还原使用受限 Win32 窗口调用，并显式排除 Dock 自身进程；两条路径都通过绝对 PowerShell 路径、动作枚举和 helper 返回值校验。UIPI、管理员权限差异及前台切换限制可能使动作失败，失败必须安全取消。
...
- 本机复测发现组件目录一次收集较多文件会停滞，而逐批测试通过；组件门禁现自动递归发现全部测试文件，按每批最多 5 个重新启动测试进程。任一批失败或超过 120 秒立即令整个门禁失败，不裁剪覆盖范围。
- Electron 合成 UI smoke 模式在 `app.ready` 前同时关闭硬件加速、GPU 与软件光栅 GPU 子进程；正常启动和真实摄像头验收保留正常 WebGL 策略，避免禁用手势模型依赖的运行环境。合成 smoke 可在无可用 GPU 的远程/隔离环境中测量窗口与资源预算。
...
- 自动更新仅支持 NSIS 安装版；新客户端按用户要求接收所有较新版本（包含明确标注的未签名预览版），禁止降级。签名生产发布仍失败关闭，未签名通道不代表发布者身份认证；便携版保持手动更新。
