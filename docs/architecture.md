...
模式切换时扫码循环立即停止，手势循环只在姿态模式运行，避免与扫码争用主线程。姿态流继续拥有摄像头生命周期，因此结束会话仍能统一停止所有轨道。

折叠与展开只改变 Electron 主窗口边界和 React 布局，不卸载 `HTMLVideoElement`，因此摄像头流、姿态会话和当前工具模式在 `348 × 360` 迷你 Dock 与 `700 × 680` 完整面板之间连续保留。窗口尺寸变化继续以原窗口右下角为锚点并限制在当前显示器工作区内。
...
- 所有新窗口和外部导航默认拒绝；摄像头权限只授予主窗口的视频请求。
