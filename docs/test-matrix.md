...
## 2026-09-19 Windows UI 清晰度

- 六手势映射与具体手指描述；未启动、加载、异常、暂停、候选及松手状态。
- 主入口、停止摄像头、暂停、语音与模式切换的回调；语音保持默认关闭。
- Windows 模式不泄漏 Codex 话筒提示；Codex/空中鼠标仍可切换。
- 桌面与窄窗口视觉检查、键盘可达性、既有无障碍及 fast 门禁。网页不验证真实手势准确率或 Win32 效果。

## 2026-09-18 桌面控制增量矩阵

Windows v2 为默认手势模式；MediaPipe 手势识别循环使用 worker（其他视觉工具不在此变更范围），Windows/Codex 动作使用连续保持，空中鼠标使用 One Euro 自适应平滑。25 条简中离线语音语法已本机探针验证，中英文短语与后端语法通过一致性测试；真人手势、噪声和麦克风断开仍待人工验收。

0.6.1 候选验收补充：camera-only 不加载或等待坐姿校准；手势模型 ready 后才进入极简状态，失败不报启动成功。真实摄像头探针保留正常 WebGL、隔离配置并禁止桌面动作，不保存画面；只证明真实帧与模型运行，不证明真人六手势准确率。原生测试抽取正式 helper 的 C#，只在自建窗口测试 F24、最大化、还原、最小化和 PID 排除。更新测试使用实际 GitHubProvider 配合模拟 feed/元数据，验证版本与下载路径，不冒充真实安装或签名认证。

| 场景 | 预期结果 | 类型 | 自动化 |
|---|---|---|---|
| 六手势固定动作 | 胜利/ILY/握拳/拇指/食指/张掌分别映射下一个窗口、上一个窗口、最小化、最大化或还原、任务视图、显示桌面；确认、释放和 700ms 限流稳定 | Unit + Electron + Windows manual | 是 / 发布前实机 |
...
| 本机语音命令 | 每次启动默认关闭；只有可信主窗口可启停；helper 只加载文化匹配的 25 条固定 Grammar，只输出白名单动作/固定短语/相对置信度；非法 JSON、未知动作、自由文本、超长值、低置信度和重复过快命令失败关闭；10 秒启动超时；关闭、异常与退出结束进程且不无限重启；无识别器/默认麦克风显示可恢复状态；语音不能处理审批或绕过 Windows 急停 | Unit + Electron integration + Component + Windows manual | 部分 / 简中与英文 grammar 已探针验证；真人语音、噪声、断开麦克风仍待人工复核 |
