...
| OCR 置信度复核 | 摘要平均/最低/阈值下数量正确；非有限数据与无效框过滤，越界框限制，低分排序和 100 项上限稳定；文件、名片、扫描页可进入/返回原图叠框视图，50–95 阈值即时更新且不修改文字；无词框的 PDF 文本层不显示入口，对象 URL 释放 | Unit + Component + Browser/manual | 是 / 发布前真实低清图像复核 |
| OCR 版面 JSON/CSV | schema v1、行分组、词序、置信度、像素/归一化坐标、越界限制与无效框过滤稳定；文件名净化，JSON 换行和 UTF-8 BOM/CRLF/引号/公式中和正确；文件、名片、扫描页只在有词框时显示入口，文件尺寸按需解码、扫描页复用已知尺寸，对象 URL 释放且人工改字不改变原框 | Unit + Component + Browser/manual | 是 / 发布前真实导出文件复核 |
...
| 单图裁剪旋转 | 旋转归一化和 90° 宽高交换、8192 边/2400 万像素工作栅格、响应式百分比到源栅格映射、越界限制、8 像素最小区域和安全文件名正确；自由/1:1/4:3/3:4/16:9、触控/指针/键盘选择、三分法、左右旋转重置、PNG/JPEG/WebP 预览与确认导出受控；对象 URL 释放且源文件不覆盖 | Unit + Component + Browser/manual | 是 / 发布前键盘、触控、透明图与四方向复核 |
...
| 长图拼接与拆分 | 拼接 2–12 张/单张 35 MB/合计 160 MB、拆分单张 35 MB、类型/8000 万解码像素/安全文件名边界正确；最小交叉轴不放大、纵横方向、0/8/24 间距、白/深/透明背景、后续 0%–50% 裁去、顺序/移除禁用及 8192 边/2400 万像素缩放确定；拆分 2–12 份相邻边界无缺口/重复且每份预算正确；逐项进度/取消、对象 URL 释放、严格 PNG、逐份/批量明确导出受控 | Unit + Component + Browser/manual | 是 / 发布前不同尺寸、透明 PNG、手工接缝、横向拼接、奇数像素拆分、多文件下载确认与窄窗口复核 |
| 图片调整 | PNG/JPEG/WebP/BMP、35 MB/8000 万源像素、1600 边/240 万预览及 8192 边/2400 万输出边界正确；曝光、对比度、色温、饱和度、灰度固定顺序及 alpha 保留确定，六种预设与中性原样像素正确；设置变化清除旧预览、滑动对照键盘可调、取消/对象 URL 释放、JPEG 白底、严格 MIME、安全文件名、预览后明确导出受控 | Unit + Component + Browser/manual | 是 / 发布前真实照片、透明 PNG、极值设置、JPEG/WebP 编码、长任务取消与窄窗口复核 |
...
| 颜色实验室 | 文件类型/35 MB、2400 边长与 400 万像素限制；透明像素白底合成；OKLCH 六色提取、坐标取样、HEX/RGB 转换、WCAG 2 比率和 AA/AAA 阈值正确；前景/背景切换、交换、手工输入与 CSS/JSON 复制受控 | Unit + Component + Browser/manual | 是 / 发布前真实照片取样与键盘复核 |
