...
- **0.6.2 更新**：Windows 控制入口、暂停及六个手势说明前置；完整显示动作名、具体手指摆法与窗口效果。修正 Windows 食指动作与 Codex 话筒说明冲突，并优化窄窗口布局。
- 摄像头会话现在默认仅识别手部，不加载坐姿模型、不要求脸或身体入镜、不进行坐姿校准。“坐姿提醒”是单独主动开启的功能；切换其他工具或手势模式后回到不检测坐姿的状态。
...
- 保留中英文各 25 条本机语音命令。在“语音控制”区点击开启语音（小窗使用顶部麦克风）后可说“助手 切换窗口”“助手 最小化窗口”“助手 最大化窗口”“助手 暂停控制”；“助手 缩小悬浮窗”只缩小 Dock。语音每次启动默认关闭，不上传录音，极简浮钮显示监听标记。
