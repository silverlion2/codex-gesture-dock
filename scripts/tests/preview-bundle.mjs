...
test('unsigned preview bundle is allowlisted, hash-verified and refuses overwrite', {
  ...
  try {
    ...
    const names = ['Codex-Gesture-Dock-1.2.3-setup.exe', 'Codex-Gesture-Dock-1.2.3-portable.exe', 'sbom.cdx.json']
    const bytes = Buffer.from('test fixture, never executed')
    const sha256 = createHash('sha256').update(bytes).digest('hex')
    for (const name of [...names, 'latest.yml', 'installer.blockmap']) {
    ...
    assert.equal(manifest.tag, 'preview/v1.2.3')
    assert.equal(manifest.commit, 'a'.repeat(40))
    assert.equal(manifest.unsigned, true)
    assert.equal(manifest.manualDownloadOnly, true)
